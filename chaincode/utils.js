...
class utils
{
     ...
     * @param {*} companyCRN 
     */
    async fetchCompanyName(ctx, companyCRN){
        var companyKey = await ctx.stub.createCompositeKey('company.', [companyCRN]);
        var companyBuffer = await ctx.stub.getState(companyKey);
        var companyName = companyBuffer.toString();
        return companyName;
    }
