...
PEER0_REGISTRAR_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/registrar.pharma-network.com/peers/peer0.registrar.pharma-network.com/tls/ca.crt
PEER0_USERS_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/users.pharma-network.com/peers/peer0.users.pharma-network.com/tls/ca.crt
...
setGlobals() {
  ...
  if [ "$ORG" == 'registrar' ]; then
    ...
    CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/users.pharma-network.com/users/Admin@users.pharma-network.com/msp
    if [ "$PEER" -eq 0 ]; then
...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    ...
    set -x
    peer chaincode instantiate -o orderer.pharma-network.com:7050 --tls "$CORE_PEER_TLS_ENABLED" --cafile $ORDERER_CA -C $CHANNEL_NAME -n pharmanet -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["org.pharma-network.pharmanet-users:instantiate"]}' -P "OR ('registrarMSP.member','usersMSP.member')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
