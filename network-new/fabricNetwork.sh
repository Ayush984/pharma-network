...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/registrar.pharma-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/REGISTRAR_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
  cd crypto-config/peerOrganizations/users.pharma-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile pharmachannel -outputAnchorPeersUpdate ./channel-artifacts/registrarMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg registrarMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile pharmachannel -outputAnchorPeersUpdate ./channel-artifacts/usersMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg usersMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
